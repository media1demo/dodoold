...
export async function onRequest(context) {
    ...
    if (url.pathname === '/api/webhook' && method === 'POST') {
        try {
            ...
            // Convert Headers object to plain object for webhook verification
            const headers = {};
    ...
    // --- ROUTE 4: KV TEST (Remove this after testing) ---
    if (url.pathname === '/test-kv' && method === 'GET') {
        try {
            // Test write
            const testData = {
                test: true,
            ...
            await kv.put('test-key', JSON.stringify(testData));
            console.log('[KV Test] Write successful');
            
            // Test read
            const retrieved = await kv.get('test-key', { type: 'json' });
            console.log('[KV Test] Read successful:', JSON.stringify(retrieved));
            
            const resultHtml = `
                <h1>KV Storage Test</h1>
                <div class="product-card">
                    <h3>✅ Write Test: Success</h3>
                    <p>Written data: ${JSON.stringify(testData, null, 2)}</p>
                </div>
                <div class="product-card">
                    <h3>✅ Read Test: Success</h3>
                    <p>Retrieved data: ${JSON.stringify(retrieved, null, 2)}</p>
                </div>
                <a href="/" class="button">Go Home</a>
            `;
            return new Response(generateHtmlPage("KV Test Results", resultHtml), { 
            ...
            const errorHtml = `
                <h1>KV Storage Test</h1>
                <div class="product-card" style="border-color: red;">
                    <h3>❌ Test Failed</h3>
                ...
                <a href="/" class="button">Go Home</a>
            `;
            return new Response(generateHtmlPage("KV Test Failed", errorHtml), { 
                status: 500,
                headers: { 'Content-Type': 'text/html' } 
            });
        }
